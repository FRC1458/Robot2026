...
public class PIDToPoseCommand extends Command {
	...
	public PIDToPoseCommand(Drive drive, Pose2d target) {
		this(drive, target, TRANSLATION_CONSTANTS, PROFILED_ROTATION_CONSTANTS);
	}
	...
	public ChassisSpeeds calculateSpeeds() {
		...
		double rotation =
				MathUtil.clamp(
						thetaController
								.setTarget(target.getRotation().getRadians()) // Theta target
								.setMeasurement(
										currentPose.getRotation().getRadians(),
										currentSpeeds
												.omegaRadiansPerSecond) // We are where we are and we are as fast as how
								// fast we are going
								.getOutput(),
						-MAX_ROTATION_SPEED,
						MAX_ROTATION_SPEED);

		DogLog.log("Debug/PIDToPose/vx", vMagnitude * deltaRotation.getCos());
		DogLog.log("Debug/PIDToPose/vy", vMagnitude * deltaRotation.getSin());
		DogLog.log("Debug/PIDToPose/vrotation", rotation);
		DogLog.log("translationerror", translationController.getError());
		DogLog.log("rotationerror", thetaController.getError());
	...
	public boolean isFinished() {
		return finishDebouncer.calculate(
				Math.abs(translationController.getError()) <= EPSILON_TRANSLATION
						&& MathUtil.isNear(
								thetaController.getError(), 0, EPSILON_ROTATION)); // Within tolerance
	}
	...
	public static double estimateTimeToPose(
		...
		return Util.trapezoidProfileTimeToTarget(translationError, currentV, 0, MAX_SPEED, MAX_ACCEL);
	}
