...
public class FollowerTalonFXMotorIO implements MotorIO<FollowerTalonFXInputs> {
    ...
    private final List<StatusSignal<Angle>> followerPositionSignals = new ArrayList<>();
    private final List<StatusSignal<AngularVelocity>> followerVelocitySignals = new ArrayList<>();
    private final List<StatusSignal<Voltage>> followerVoltageSignals = new ArrayList<>();
    private final List<StatusSignal<Current>> followerCurrentStatorSignals = new ArrayList<>();
    private final List<StatusSignal<Current>> followerCurrentSupplySignals = new ArrayList<>();
    private final List<StatusSignal<Angle>> followerRawRotorPositionSignals = new ArrayList<>();
    private final List<StatusSignal<Temperature>> followerTemperatureSignals = new ArrayList<>();

    private final List<BaseStatusSignal> signals;
    private final List<List<BaseStatusSignal>> followerSignals = new ArrayList<>();
    ...
    public FollowerTalonFXMotorIO(
        CanDevice main,
        List<CanDevice> followerDevices,
        TalonFXConfiguration config,
        List<TalonFXConfiguration> followerConfigs,
        List<MotorAlignmentValue> alignmentValues,
        String name
    ) {
        this.name = name;
        size = followerDevices.size();
        assert size == followerConfigs.size() && size == alignmentValues.size();
        ...
        for (int i = 0; i < followerDevices.size(); i++) {
            TalonFX followerMotor = new TalonFX(followerDevices.get(i).id(), followerDevices.get(i).bus());
            CtreUtil.applyConfiguration(followerMotor, followerConfigs.get(i));
            CtreUtil.applyFollower(main.id(), followerMotor, alignmentValues.get(i));
            followers.add(followerMotor);

            followerPositionSignals.add(followerMotor.getPosition());
            followerVelocitySignals.add(followerMotor.getVelocity());
            followerVoltageSignals.add(followerMotor.getMotorVoltage());
            followerCurrentStatorSignals.add(followerMotor.getStatorCurrent());
            followerCurrentSupplySignals.add(followerMotor.getSupplyCurrent());
            followerRawRotorPositionSignals.add(followerMotor.getRotorPosition());
            followerTemperatureSignals.add(followerMotor.getDeviceTemp());

            followerSignals.add(
                List.of(
                    followerPositionSignals.get(i),
                    followerVelocitySignals.get(i),
                    followerVoltageSignals.get(i),
                    followerCurrentStatorSignals.get(i),
                    followerCurrentSupplySignals.get(i),
                    followerRawRotorPositionSignals.get(i),
                    followerTemperatureSignals.get(i)));
        }
    }

    @Override
    public void readInputs(FollowerTalonFXInputs inputs) {
        BaseStatusSignal.refreshAll(signals);

        for (List<BaseStatusSignal> followerSignals : followerSignals) {
            BaseStatusSignal.refreshAll(followerSignals);
        }
        ...
        for (int i = 0; i < inputs.inputs.length; i++) {
            TalonFXInputsAutoLogged follower = inputs.inputs[i];

            follower.position.mut_replace(followerPositionSignals.get(i).getValue());
            follower.velocity.mut_replace(followerVelocitySignals.get(i).getValue());
            follower.appliedVoltage.mut_replace(followerVoltageSignals.get(i).getValue());
            follower.statorCurrent.mut_replace(followerCurrentStatorSignals.get(i).getValue());
            follower.supplyCurrent.mut_replace(followerCurrentSupplySignals.get(i).getValue());
            follower.rawRotorPosition.mut_replace(followerRawRotorPositionSignals.get(i).getValue());
            follower.temperature.mut_replace(followerTemperatureSignals.get(i).getValue());
        }
    }
