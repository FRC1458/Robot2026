...
public class RightIndexer extends RollerMotorSubsystem {
	public RightIndexer() {
		super(
				() -> {
					TalonFX motor = new TalonFX(R_MOTOR_ID);

					if (RobotBase.isReal()) {
						return new ITalonFX(motor, "Indexer/Right");
					} else {
	...
	public Command index() {
		return runVel(FEEDER_SPEED, RotationsPerSecond.of(10), RunMode.VOLTAGE).withTimeout(1);
	}
