...
public class FieldLayout {
	...
	static {
		...
		// 	DriveConstants.TRACK_WIDTH / 2 + Units.inchesToMeters(1.5), -Units.inchesToMeters(13.0 / 2),
		// Rotation2d.kPi);
	...
	public static Pose2d handleAllianceFlip(Pose2d blue_pose, boolean is_red_alliance) {
		if (is_red_alliance) {
			blue_pose =
					new Pose2d(
							new Translation2d(
									(FIELD_LENGTH / 2.0) + ((FIELD_LENGTH / 2.0) - blue_pose.getX()),
									blue_pose.getY()),
	...
	public static Translation2d handleAllianceFlip(
		...
		if (is_red_alliance) {
			blue_translation =
					new Translation2d(
							(FIELD_LENGTH / 2.0) + ((FIELD_LENGTH / 2.0) - blue_translation.getX()),
							blue_translation.getY());
		}
