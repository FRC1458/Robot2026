...
public class FieldLayout {
	...
	public static final List<Pose2d> TARGET_HUB_POSES = new ArrayList<>();
	...
	static {
		try {
			...
			int[] hubIds = {9,10,25,26};
			Transform2d moveOut = new Transform2d(
				Units.inchesToMeters(45), 0, Rotation2d.kZero); //TODO: tune distance
			for (int i : hubIds) {
				var tagPose = APRILTAG_MAP.getTagPose(i).get().toPose2d();
				TARGET_HUB_POSES.add(tagPose.transformBy(moveOut));
			}
		} catch (IOException e) {
	...
	public static APTarget getHubTarget(Pose2d pose) {
		var set = TARGET_HUB_POSES;
