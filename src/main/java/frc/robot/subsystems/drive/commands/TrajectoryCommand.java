...
/** Command that follows a trajectory TODO: fix the profiled pid controller */
public class TrajectoryCommand extends Command {
	...
	public TrajectoryCommand(Drive drive, RedTrajectory trajectory) {
		this(
				drive,
				trajectory,
	...
	/**
	 * A drive controller that works with 2 {@link PIDVController}s for translation and one {@link
	 * PIDVController} for rotation.
	 *
	 ...
	 * @param accelConstant The acceleration feedforwards (useful for traversing sharp turns on a
	 *     trajectory).
	 */
	public TrajectoryCommand(
			Drive drive,
			RedTrajectory trajectory,
			PIDVConstants translationConstants,
			ProfiledPIDVConstants rotationConstants,
		...
		TelemetryManager.getInstance()
				.addStructPublisher(
						"Debug/TrajectoryCommand", Pose3d.struct, () -> new Pose3d(targetState.pose));
	...
	public Command withPIDToPoseAtEnd() {
		return this.andThen(
				new PIDToPoseCommand(drive, trajectory.getFinalState().pose)
	...
	public void initialize() {
		...
		thetaController.setInitialSetpoint(
				drive.getPose().getRotation().getRadians(), drive.getFieldSpeeds().omegaRadiansPerSecond);
	}

	@Override
	public void execute() {
		setRobotState(drive.getPose(), drive.getFieldSpeeds());
	...
	public ChassisSpeeds calculateSpeeds() {
		...
		double vx =
				xController
						.setTarget(
								targetState.pose.getX(), targetState.speeds.vxMetersPerSecond) // Target setting
						.setMeasurement(
								currentPose.getX(), currentSpeeds.vxMetersPerSecond) // Measurement setting
	...
	private static class RMSTracker {
		...
		private void updateRmsError(
			...
			double translationError = Math.hypot(xController.getError(), yController.getError());
			translationErrorIntegralSq += translationError * translationError * dt;
			double rotationError = rotationController.getError();
