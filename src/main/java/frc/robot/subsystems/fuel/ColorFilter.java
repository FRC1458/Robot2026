...
public class ColorFilter {
    ...
    double distance = -VisionConstants.CAMHEIGHT / (Math.tan(VisionConstants.CAMANGLE + VisionConstants.FOVVE * ycoord));

    double desiredx = Drive.getInstance().getPose().getX() + distance * Math.sin((Rotation2d.fromDegrees(desiredrotation).plus(m_angle)).getRadians());
    double desiredy = Drive.getInstance().getPose().getY() + distance * Math.sin((Rotation2d.fromDegrees(desiredrotation).plus(m_angle)).getRadians());
    ...
    public Command goToDesired() {
        ...
        */
        return Commands.defer(() -> { //yeah idk what command to use here
            Pose2d desiredpose = new Pose2d(desiredx,desiredy,m_angle); //rotation doesn't matter
            APTarget target = new APTarget(desiredpose);
            return new AutopilotCommand(target);
        }
        ).withName("autopilotting to desired pose or whatever");
