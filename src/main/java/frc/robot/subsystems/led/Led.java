...
public class Led extends SubsystemBase {
	...
	/** Updates the LEDs */
	private void update() {
		if (colorer.state != Colorer.State.TIMED_PATTERN) {
			if (colorer.hasUpdated) {
	...
	public Command setSolidColorCommand(Color color, double holdTime) {
		return runOnce(() -> colorer.setSolidColor(color))
				.andThen(Commands.waitSeconds(holdTime))
	...
	public Command setRainbowCommand(double holdTime) {
		return runOnce(
						() ->
								colorer.setTimedPattern(
										(Integer index, Double time) ->
												Color.fromHSV(index * 5 + (int) (time * 50), 255, 255)))
				.andThen(Commands.waitSeconds(holdTime))
				.ignoringDisable(true)
				.withName("Rainbow");
	}
	...
	public Command setRandomCommand(double holdTime) {
		return defer(
						() -> {
							...
							return runOnce(() -> colorer.setPattern(colors));
						})
				.andThen(Commands.waitSeconds(holdTime))
				.ignoringDisable(true)
				.withName("Random");
	...
	public Command blinkCommand(Color color1, Color color2, double delta, double holdTime) {
		return Commands.repeatingSequence(
						setSolidColorCommand(color1, delta), setSolidColorCommand(color2, delta))
				.raceWith(Commands.waitSeconds(holdTime))
				.withName(
						color1.toString() + ":" + color2.toString() + ":" + (int) (delta * 1000) + " ms Blink");
	}
}
